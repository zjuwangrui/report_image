...
def load_and_transform_data(file_path: Path) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray]]:
    ...
    Returns:
        A tuple of (phi_m, phi_sq, I) numpy arrays, or None if loading fails.
    """
    ...
    phi_mm = df['phi_mm'].values
    I = df['I'].values
    
    # Convert phi from mm to m for SI unit consistency
    phi_m = phi_mm / 1000.0
    
    # Light intensity is proportional to the area of the aperture, which is proportional to phi².
    # We will verify the linear relationship between I_s and phi².
    phi_sq = phi_m ** 2
    
    simple_logger("数据转换完成: phi (mm) -> phi² (m²)。")
    return phi_m, phi_sq, I
...
def plot_and_save_graph(
    ...
    """
    simple_logger(f"正在绘制 I vs phi² 关系图...")
    ...
    fit_equation = f'$I_s = {slope:.4f} \\cdot \\phi^2 + {intercept:.4f}$'
    plt.plot(x_fit, y_fit, color='red', label=f'Linear Fit\n{fit_equation}\n$R^2 = {r_squared:.4f}$')
    
    plt.title('Saturation Photocurrent vs. (Aperture Diameter)$^2$')
    plt.xlabel('$\\phi^2$ ($m^2$)')
...
def save_report_file(
    ...
    report_content = (
        f"饱和光电流与光阑孔径关系验证报告\n"
        f"========================================\n"
        f"实验目的: 验证饱和光电流 $I_s$ 与光强 $E$ 的正比关系，其中 $E \\propto Area \\propto \\phi^2$。\n"
        f"因此，我们检验 $I_s$ 与 $\\phi^2$ 的线性关系。\n\n"
        f"分析结果:\n"
        f"  - 线性拟合的决定系数 (R²): {r_squared:.4f}\n"
        f"  - 拟合方程: I_s = {slope:.4f} * (phi²) + {intercept:.4f}\n\n"
        f"结论: R² 值为 {r_squared:.4f}，非常接近1，表明饱和光电流 $I_s$ 与 $\\phi^2$ 之间有很强的线性关系，\n"
        f"从而验证了饱和光电流与光强成正比。\n"
        f"========================================\n"
        f"原始输入数据:\n"
        f"phi(m), I(10e-10 A)\n"
    ...
    for p, i in zip(phi_m, I):
        report_content += f"{p:.4f}, {i}\n"
...
def main() -> None:
    ...
    try:
        ...
        # 定义路径
        base_path = Path('d:/report_image/ex3/output/phiI_image')
        ...
        # 线性回归
        slope, intercept, r_squared = perform_linear_regression(phi_sq, I)
        
        # 绘图
        plot_and_save_graph(phi_sq, I, slope, intercept, r_squared, image_output_path)
        
        # 保存报告
        save_report_file(report_output_path, r_squared, slope, intercept, phi_m, I)
